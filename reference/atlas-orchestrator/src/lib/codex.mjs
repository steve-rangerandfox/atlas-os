...
export async function runCodexTask({ mission, task }) {
  ...
  try {
    const result = await runProcess(config.codexBin, args, {
      cwd: mission.workDir,
      input: buildExecutorPrompt(mission, task),
    ...
    try {
      ...
      throw new OrchestratorError(
        ...
        "CODEX_INVALID_REPORT",
        { stdout: redactText(result.stdout), stderr: redactText(result.stderr) }
      );
    }
    if (result.code !== 0 || result.timedOut) {
      throw new OrchestratorError(
        `Codex exited with code ${result.code}${result.timedOut ? " after timing out" : ""}`,
        result.timedOut ? "CODEX_TIMEOUT" : "CODEX_FAILED",
        { report: redactObject(report), stderr: redactText(result.stderr) }
      );
    }
    return {
      report: redactObject(report),
